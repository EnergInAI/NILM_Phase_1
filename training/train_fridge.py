...
ON_THRESHOLD = 0.05          # kW
ON_WEIGHT = 3.0              # ON samples importance
PATIENCE = 6
...
for b in TRAIN_BUILDINGS:
    ...
    fridge_df = read_meter(H5_PATH, b, FRIDGE_METER)

    mains = preprocess_df(mains_df, normalize=True)
    fridge = preprocess_df(fridge_df, normalize=False) / 1000.0  # kW

    min_len = min(len(mains), len(fridge))
    mains = mains[:min_len]
    fridge = fridge[:min_len]

    X, y = create_seq2point(mains, fridge)
